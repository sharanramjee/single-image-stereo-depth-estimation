...
def generate_stereo(in_dir, depth_dir, depth_prefix, out_dir, filename):
    ...
    # left = cv2.imread(os.path.join(in_dir, filename + ".png"))
    left = io.imread(os.path.join(in_dir, filename + ".png"))
    # depth_src = cv2.imread(os.path.join(depth_dir, depth_prefix + filename + ".png"))
    d_f_split = filename.split('_')
    depth_f_name = d_f_split[0] + '_depth_' + d_f_split[2]
    depth_src = io.imread(os.path.join(depth_dir, depth_prefix + depth_f_name + ".png"))
    ...
    cv2.imwrite(os.path.join(out_dir, "right_" + filename + ".jpg"), right_fix)
